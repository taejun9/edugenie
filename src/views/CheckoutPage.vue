<template>
  <div class="min-h-screen bg-gray-50">
    <Header />

    <main class="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      <!-- Back link -->
      <router-link
        to="/"
        class="mb-6 inline-flex items-center gap-1 text-sm text-gray-400 transition-colors hover:text-gray-600"
      >
        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
        </svg>
        Back to Pricing
      </router-link>

      <h1 class="mb-8 text-2xl font-extrabold text-gray-900">Checkout</h1>

      <div class="grid gap-8 lg:grid-cols-5">
        <!-- Order Summary -->
        <div class="lg:col-span-2">
          <div class="sticky top-24 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <h2 class="mb-4 font-bold text-gray-900">Order Summary</h2>
            <div class="mb-4 rounded-xl bg-blue-50 p-4">
              <p class="text-sm font-semibold text-color-main">Pro Plan</p>
              <p class="mt-1 text-2xl font-extrabold text-gray-900">
                $12
                <span class="text-sm font-normal text-gray-500">/ month</span>
              </p>
            </div>
            <ul class="!ml-0 space-y-2">
              <li class="flex items-center gap-2 text-sm text-gray-600">
                <svg
                  class="h-4 w-4 text-color-main"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  stroke-width="2.5"
                >
                  <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                Unlimited lessons
              </li>
              <li class="flex items-center gap-2 text-sm text-gray-600">
                <svg
                  class="h-4 w-4 text-color-main"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  stroke-width="2.5"
                >
                  <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                All quiz types
              </li>
              <li class="flex items-center gap-2 text-sm text-gray-600">
                <svg
                  class="h-4 w-4 text-color-main"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  stroke-width="2.5"
                >
                  <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                </svg>
                Export to PDF / Word
              </li>
            </ul>
            <div class="mt-4 border-t border-gray-100 pt-4">
              <div class="flex justify-between text-sm">
                <span class="text-gray-500">Subtotal</span>
                <span class="font-bold text-gray-900">$12.00</span>
              </div>
            </div>
          </div>
        </div>

        <!-- Payment Form (placeholder) -->
        <div class="lg:col-span-3">
          <div class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <h2 class="mb-6 font-bold text-gray-900">Payment Details</h2>

            <div class="space-y-4">
              <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Card Number</label>
                <input
                  type="text"
                  placeholder="1234 5678 9012 3456"
                  disabled
                  class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-400"
                />
              </div>
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="mb-1 block text-sm font-medium text-gray-700">Expiry</label>
                  <input
                    type="text"
                    placeholder="MM / YY"
                    disabled
                    class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-400"
                  />
                </div>
                <div>
                  <label class="mb-1 block text-sm font-medium text-gray-700">CVC</label>
                  <input
                    type="text"
                    placeholder="123"
                    disabled
                    class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-400"
                  />
                </div>
              </div>
              <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">Name on Card</label>
                <input
                  type="text"
                  placeholder="Hong Gildong"
                  disabled
                  class="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-400"
                />
              </div>
            </div>

            <button
              disabled
              class="mt-8 w-full cursor-not-allowed rounded-xl bg-gray-200 py-3.5 text-sm font-semibold text-gray-400"
            >
              Payment integration coming soon
            </button>

            <p class="mt-4 text-center text-xs text-gray-400">
              🔒 Payments are securely processed. Your card information is encrypted.
            </p>
          </div>
        </div>
      </div>
    </main>
  </div>
</template>

<script setup>
import Header from '@/components/Layout/Header.vue'
</script>
