<template>
  <div class="group relative inline-block">
    <svg
      viewBox="0 0 24 24"
      class="h-full w-full"
      :class="isTurnOff ? `fill-gray-200` : `fill-yellow-200`"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        id="bulb"
        :class="isTurnOff ? `stroke-black` : `stroke-yellow-600`"
        d="M12 7C9.23858 7 7 9.23858 7 12C7 13.3613 7.54402 14.5955 8.42651 15.4972C8.77025 15.8484 9.05281 16.2663 9.14923 16.7482L9.67833 19.3924C9.86537 20.3272 10.6862 21 11.6395 21H12.3605C13.3138 21 14.1346 20.3272 14.3217 19.3924L14.8508 16.7482C14.9472 16.2663 15.2297 15.8484 15.5735 15.4972C16.456 14.5955 17 13.3613 17 12C17 9.23858 14.7614 7 12 7Z"
        stroke-width="2"
      />
      <path
        d="M12 4V3"
        :class="isTurnOff ? `hidden` : `stroke-yellow-400`"
        stroke-width="2"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        d="M18 6L19 5"
        :class="isTurnOff ? `hidden` : `stroke-yellow-400`"
        stroke-width="2"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        :class="isTurnOff ? `hidden` : `stroke-yellow-400`"
        d="M20 12H21"
        stroke-width="2"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        d="M4 12H3"
        :class="isTurnOff ? `hidden` : `stroke-yellow-400`"
        stroke-width="2"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        :class="isTurnOff ? `hidden` : `stroke-yellow-400`"
        d="M5 5L6 6"
        stroke-width="2"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        id="bulb-middle"
        :class="isTurnOff ? `stroke-black` : `stroke-yellow-600`"
        d="M10 17H14"
        stroke-width="2"
        stroke-linecap="round"
        stroke-linejoin="round"
      />
      <path
        id="bulb-head"
        :class="isTurnOff ? `stroke-black` : `stroke-yellow-600`"
        opacity="0.1"
        d="M7 12C7 9.23858 9.23858 7 12 7C14.7614 7 17 9.23858 17 12C17 13.3613 16.456 14.5955 15.5735 15.4972C15.2297 15.8484 14.9472 16.2663 14.8508 16.7482L14.8004 17H9.19961L9.14923 16.7482C9.05281 16.2663 8.77025 15.8484 8.42651 15.4972C7.54402 14.5955 7 13.3613 7 12Z"
      />
    </svg>
    <div
      class="absolute -bottom-6 -left-2/3 z-50 hidden rounded-xl border border-gray-300 bg-[#999] px-2 py-1 text-center text-xs font-semibold text-white group-hover:block"
    >
      {{ isTurnOff ? 'USED' : 'REMAIN' }}
    </div>
  </div>
</template>

<script setup>
defineProps({
  isTurnOff: {
    type: Boolean,
    required: false,
  },
})
</script>
