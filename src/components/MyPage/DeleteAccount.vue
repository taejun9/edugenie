<template>
  <div class="space-y-6">
    <div class="mb-4">
      <h2 class="text-xl font-bold text-gray-900">Delete Account</h2>
      <p class="text-sm text-gray-500">Manage your account data and privacy</p>
    </div>

    <div class="rounded-2xl border border-red-100 bg-red-50 p-6">
      <div class="flex items-start gap-4">
        <div
          class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-100 text-red-600"
        >
          <svg
            class="h-6 w-6"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="1.5"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
              d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"
            />
          </svg>
        </div>
        <div>
          <h3 class="font-bold text-red-800">Warning: Proceed with caution</h3>
          <p class="mt-2 text-sm text-red-700">
            Deleting your account is permanent. This action cannot be undone and will permanently
            delete your profile, all your generated lessons, and payment history.
          </p>
          <button
            class="mt-4 rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-red-700"
          >
            Delete My Account
          </button>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup></script>
